...
optim.evtdet.anomalize <- function(test,par_options=expand.grid(method_time_decompose=c("stl","twitter"),
  ...
  for(par in 1:nrow(par_options)){
    ...
    eval_par <- evaluate(events_a, reference, metric="F1")
    
    events_a_list[[par]] <- events_a
    eval <- rbind(eval,cbind(par_options[par,],F1=eval_par))
  }
  
  events <- events_a_list[[which.max(eval$F1)]]
  par <- par_options[which.max(eval$F1),]
  rank <- eval
  
  return(list(par=par_options[which.max(eval$F1),],
         events=events_a_list[[which.max(eval$F1)]],
         rank=rank))
}
...
optim.evtdet.changepoints_v1 <- function(test,par_options=expand.grid(w=seq(0.01,0.1,0.02)*nrow(test)),...){
  ...
  events_v1_list <- NULL
  for(par in 1:nrow(par_options)){
    events_v1 <- evtdet.changepoints_v1(test,
                                  w=par_options[par,"w"],...)
    eval_par <- evaluate(events_v1, reference, metric="F1")
    
    events_v1_list[[par]] <- events_v1
    eval <- rbind(eval,cbind(par_options[par,],F1=eval_par))
  }
  
  events <- events_v1_list[[which.max(eval$F1)]]
  par <- par_options[which.max(eval$F1),]
  rank <- eval
...
optim.evtdet.changepoints_v3 <- function(test,par_options=expand.grid(mdl_fun=c(ARIMA = function(data) forecast::auto.arima(data),
  ...
  for(par in 1:nrow(par_options)){
    #browser()
    events_v3 <- evtdet.changepoints_v3(test,
                                        mdl_fun=par_options[[par,"mdl_fun"]],
                                        m=par_options[par,"m"],...)
    eval_par <- evaluate(events_v3, reference, metric="F1")
    
    events_v3_list[[par]] <- events_v3
    eval <- rbind(eval,cbind(par_options[par,],F1=eval_par))
  }
  
  events <- events_v3_list[[which.max(eval$F1)]]
  par <- par_options[which.max(eval$F1),]
  rank <- eval
...
evtdet.mdl_outlier <- function(data,...){
  ...
  mdl_outlier <- function(data,mdl,na.action=na.omit,...){
    #browser()
    serie_name <- names(data)[-1]
    names(data) <- c("time","serie")
    ...
    #Returns index of windows with outlier error differences
    index.cp <- outliers.index(errors)
...
evtplot <- function(data, events, reference=NULL, mark.cp=FALSE, ylim=NULL,...){
  ...
  if(mark.cp & nrow(events_cp)>0) 
    tryCatch(plot <- plot + geom_segment(aes(x=time, y=top_1, xend=time, yend=bottom_1),data=events_cp, col="grey", size = .5, linetype="dashed"),
  ...
  tryCatch(plot <- plot + geom_segment(aes(x=time, y=top_1, xend=time, yend=top_2),data=ref_true, col="blue"),
           error = function(e) NULL)
  tryCatch(plot <- plot + geom_segment(aes(x=time, y=top_1, xend=time, yend=top_2),data=events_true, col="red"),
           error = function(e) NULL)
  ...
  tryCatch(plot <- plot + geom_segment(aes(x=time, y=bottom_1, xend=time, yend=bottom_2),data=ref_true, col="blue"),
           error = function(e) NULL)
  tryCatch(plot <- plot + geom_segment(aes(x=time, y=bottom_1, xend=time, yend=bottom_2),data=events_true, col="red"),
           error = function(e) NULL)
  ...
  tryCatch(plot <- plot + geom_point(aes(x=time, y=serie),data=data_ref_true, col="blue"),
           error = function(e) NULL)
  tryCatch(plot <- plot + geom_point(aes(x=time, y=serie),data=data_events_true, col="red"),
           error = function(e) NULL)
  tryCatch(plot <- plot + geom_point(aes(x=time, y=serie),data=data_ref_events_true, col="green"),
           error = function(e) NULL)
